...
ALTER TABLE public.wedding_details ADD CONSTRAINT wedding_details_theme_id_check
  CHECK (theme_id IN ('legacy','editorial','minimal','modern-noir','art-deco','boho','default'));

CREATE OR REPLACE FUNCTION public.create_new_event(_slug text, _event_type text DEFAULT 'wedding'::text, _primary_name text DEFAULT 'A definir'::text, _secondary_name text DEFAULT 'A definir'::text, _event_date date DEFAULT (((now())::date + '1 year'::interval))::date, _theme_id text DEFAULT 'legacy'::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_theme NOT IN ('legacy', 'editorial', 'minimal', 'modern-noir', 'art-deco', 'boho') THEN
    v_theme := 'legacy';
  END IF;
