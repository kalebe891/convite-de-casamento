...
ALTER TABLE public.wedding_details
  ...
  CHECK (theme_id = ANY (ARRAY['legacy'::text, 'editorial'::text, 'minimal'::text, 'modern-noir'::text, 'art-deco'::text]));
...
BEGIN
  ...
  IF v_theme NOT IN ('legacy', 'editorial', 'minimal', 'modern-noir', 'art-deco') THEN
    v_theme := 'legacy';
  END IF;
