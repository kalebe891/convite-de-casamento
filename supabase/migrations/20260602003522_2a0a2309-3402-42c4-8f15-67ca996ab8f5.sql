-- Backfill and enforce theme_id for tenants
ALTER TABLE public.wedding_details ALTER COLUMN theme_id SET DEFAULT 'legacy';

UPDATE public.wedding_details
SET theme_id = 'legacy'
WHERE theme_id IS NULL OR theme_id NOT IN ('legacy', 'editorial');

ALTER TABLE public.wedding_details ALTER COLUMN theme_id SET NOT NULL;
...
ALTER TABLE public.wedding_details
  ...
  CHECK (theme_id IN ('legacy', 'editorial'));

-- Update create_new_event to use 'legacy' as default theme
...
BEGIN
  ...
  IF v_theme NOT IN ('legacy', 'editorial') THEN
    v_theme := 'legacy';
  END IF;
